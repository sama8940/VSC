...
# SSEを求める関数の定義
def get_sse():
    # データと中心点までの距離の2乗を集計する
    sse = 0
    for i in range(student_num):
        sse += (japanese_score[i] - japanese_center[group_idx[i]]) ** 2 \
        + (math_score[i] - math_center[group_idx[i]]) ** 2
    return sse
...
if __name__ == '__main__':
    ...
    # y軸のデータ（k = 1～10におけるSSE）
    y_data = []
    # k = 1～10におけるSSEを求める
    for k in x_data:
        # 中心点の初期位置を設定する
        init_center()
        # 中心点の移動がなくなるまで、グループ分けと中心点の移動を繰り返す
        center_fixed = False
        while not center_fixed:
            grouping()
            move_center()
        # SSEを求める
        y_data.append(get_sse())
    # kとSSEの値を折れ線グラフに設定する
    plt.title("エルボー法で適切なクラスタ数を判断する", fontname="MS Gothic")
    plt.xlabel("k（クラスタ数）", fontname="MS Gothic")
    plt.ylabel("SSE（誤差平方和）", fontname="MS Gothic")
    plt.xticks(x_data)
    plt.grid(True)
