...
# クイックソートと挿入ソートを組み合わせて昇順でソートを行う関数の定義
def quick_ins_sort(a, left, right):
    # 要素数が10個より大きければクイックソートで配列の二分割を繰り返す
    if right - left + 1 > 10:
        # 配列を要素の大小で二分割する
        pivot = split_array(a, left, right)

        # 分割した前側の配列に同じ処理を行う
        quick_ins_sort(a, left, pivot - 1)

        # 分割した後ろ側の配列に同じ処理を行う
        quick_ins_sort(a, pivot + 1, right)
    # そうでないなら（要素数が10個以下なら）挿入ソートでソートを完了する
    else:
...
# クイックソートと挿入ソートを組み合わせたソートをテストする
a = copy.copy(rand_data) # 乱数の配列のコピーを生成する
left = 0 # 配列の先頭の添字を左端に設定する
right = len(a) - 1 # 配列の末尾の添字を右端に設定する
time1 = time.perf_counter() # 処理前の時間を得る
quick_ins_sort(a, left, right) # ソートを行う
time2 = time.perf_counter() # 処理後の時間を得る
# テスト結果を表示する
print("***** クイックソートと挿入ソートを組合せたソート *****")
print(f"ソート結果 = {is_asc_sorted(a)}") # 昇順にソートされている
                                         # ことを確認する
print(f"ソート時間 = {time2 - time1}秒") # ソートの処理時間を求める