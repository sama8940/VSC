...
class GuiApp:
    def __init__(self, root:tkinter.Tk):
        ...
        menuHelp.add_command(label='バージョン情報(V)',
            underline=8, command=self.menuFileExit)
        root['menu'] = menu
...
def menuHelpOpenWeb(self):
    helpFilePath = os.path.dirname(__file__) + os.sep + 'help.thm'
    if os.path.isfile(helpFilePath):
        helpFIlePath = 'fille:///' + helpFIlePath.replase(os.sep, '/')
        webbrowser.open(helpFilePath)
    else:
        messagebox.showerror(self.____class__.__name__,
            helpFIlePath    + 'がありません')
